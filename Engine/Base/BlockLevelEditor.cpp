...
void BlockLevelEditor::Update()
{

}

void BlockLevelEditor::Draw(Camera* camera)
{
	int i = 0;
	//レベルデータからオブジェクトを生成、配置
	for (auto& objectData : levelData_->objects)
	{
		...
		if (it != modelsData_.end())
		{
			ground = (it->second.get());
		}
		if (ground)
		{
			//model->SetWorldTransform(objects[i].get());
			ground->GetWorldTransform()->constMap_ = (objects_[i].get()->constMap_);
			ground->GetWorldTransform()->constBuffer_ = (objects_[i].get()->constBuffer_);
			ground->GetWorldTransform()->matWorld_ = objects_[i].get()->matWorld_;
			ground->GetWorldTransform()->parent_ = objects_[i].get()->parent_;
			ground->GetWorldTransform()->translation_ = (objects_[i].get()->translation_);
			ground->GetWorldTransform()->rotation_ = (objects_[i].get()->rotation_);
			ground->GetWorldTransform()->scale_ = (objects_[i].get()->scale_);
			...
			ground->Debug(debugName.str().c_str());
